...
import com.example.onlinemusicstreamapp.database.other.Constants.PERSONALIZED_PLAYLIST_COLLECTION
...
class PlaylistDatabase {
    ...
    private val personalizedPlaylistCollection = fireStore.collection(PERSONALIZED_PLAYLIST_COLLECTION)
    ...
    suspend fun createPlaylist(playlist: Playlist) {
        try {
            personalizedPlaylistCollection.document(playlist.playlistId).set(playlist).await()
        }
        catch (e: Exception) {
            e.printStackTrace()
        }
